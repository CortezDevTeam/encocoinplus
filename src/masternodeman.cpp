...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "mnb") { //Masternode Broadcast
        ...
        if (!mnb.IsInputAssociatedWithPubkey(chainActive.Height())) {
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
        ...
        // make sure the vout that was signed is related to the transaction that spawned the Masternode
        //  - this is expensive, so it's only done once per Masternode
        if (!pmn->IsInputAssociatedWithPubkey(chainActive.Height())) {
        ...
        if (fAcceptable) {
            ...
            // should be at least not earlier than block when 1000 EPGC tx got MASTERNODE_MIN_CONFIRMATIONS
            uint256 hashBlock = 0;
            CTransaction tx2;
            GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
            BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
            if (mi != mapBlockIndex.end() && (*mi).second) {
                CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 10000 EPG tx -> 1 confirmation
