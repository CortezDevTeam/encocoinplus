...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    //ONLY_NOT10000IFMN = 3,
    //ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 EPG at the same time
...
// Possible states for zEPG send
enum ZerocoinSpendStatus {
    ZEPG_SPEND_OKAY = 0,                            // No error
    ZEPG_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZEPG_WALLET_LOCKED = 2,                         // Wallet was locked
    ZEPG_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZEPG_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZEPG_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ...
    ZEPG_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZEPG_SPENT_USED_ZEPG = 14,                      // Coin has already been spend
    ZEPG_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZEPG_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZEpgBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZEPGOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzepgAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzEPGTracker> zepgTracker;
    ...
    void setZWallet(CzEPGWallet* zwallet);
    CzEPGWallet* getZWallet();
    bool isZeromintEnabled();
    void setZEpgAutoBackups(bool fEnabled);
    ...
    /// Get 10000 EPG output and keys which can be used for the Masternode
    ...
    /** zEPG reset */
    boost::signals2::signal<void()> NotifyzEPGReset;
...
public:
    ...
    CAmount UpdateAmount(CAmount& amountToUpdate, bool& cacheFlagToUpdate, AvailableCoinsType nCoinType, bool fUseCache, isminetype mimeType, bool fCredit = true) const;
