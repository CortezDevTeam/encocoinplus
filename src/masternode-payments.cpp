...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake, bool fZEPGStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    ...
    if (sporkManager.IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS) && budget.IsBudgetPaymentBlock(pindexPrev->nHeight + 1)) {
        ...
        masternodePayments.FillBlockPayee(txNew, nFees, fProofOfStake, fZEPGStake);
    }
}
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake, bool fZEPGStake)
{
    ...
    for(unsigned mnlevel = CMasternode::LevelValue::MIN; mnlevel <= CMasternode::LevelValue::MAX; ++mnlevel) {
        ...
        CAmount masternodePayment = GetMasternodePayment(pindexPrev->nHeight, blockValue, mnlevel, 0, fZEPGStake);
...
bool CMasternodePayments::IsScheduled(CMasternode& mn, int nNotBlockHeight, unsigned mnlevel)
{
    ...
    for (int64_t h = nHeight; h <= nHeight + 8; h++) {
        ...
        if (mapMasternodeBlocks.count(h)) {
            if (mapMasternodeBlocks[h].GetPayee(payee, mnlevel)) {
                if (mnpayee == payee) {
                    return true;
                }
            }
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    for (CMasternodePayee& payee : vecPayments)
        if (payee.nVotes >= nMaxSignatures && payee.nVotes && payee.mnlevel >= MNPAYMENTS_SIGNATURES_REQUIRED)
    ...
    for (CMasternodePayee& payee : vecPayments) {
        ...
        CAmount requiredMasternodePayment = GetMasternodePayment(nBlockHeight, payee.mnlevel, nReward, nMasternode_Drift_Count, txNew.HasZerocoinSpendInputs());
        ...
        if (payee.nVotes >= MNPAYMENTS_SIGNATURES_REQUIRED) {
            ...
            //CBitcoinAddress address2(address1);
            auto address2 = std::to_string(payee.mnlevel) + ":" + CBitcoinAddress{address1}.ToString().c_str();
